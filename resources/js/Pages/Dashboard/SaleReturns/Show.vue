<template>
    <DashboardLayout>
        <Head :title="`Detail Return Penjualan ${saleReturn.invoice}`" />

        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Detail Return Penjualan</h1>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="text-sm font-medium text-slate-500 dark:text-slate-400">Customer</label>
                ...
                <div>
                    ...
                    <p class="text-lg font-semibold text-slate-900 dark:text-white mt-1">
                        {{ new Date(saleReturn.created_at).toLocaleString('id-ID') }}
                    </p>
                </div>
                <div>
                    ...
                    <p class="mt-1">
                        <span
                            :class="[
                                ...
                                saleReturn.status === 'finalized'
                                    ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                                    : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
                            ]"
                        >
                            {{ saleReturn.status === 'finalized' ? 'Finalized' : 'Draft' }}
                        </span>
                    </p>
                </div>
            </div>
        ...
        <TableCard title="Detail Item">
            <Table>
                ...
                <TableTbody>
                    <tr
                        v-for="(item, index) in saleReturn.details"
                        :key="item.id"
                    ...
                    <tr class="bg-slate-50 dark:bg-slate-800 font-bold">
                        ...
                        <TableTd class="text-right text-primary-600 dark:text-primary-400 text-lg">
                            {{ formatCurrency(saleReturn.grand_total) }}
                        </TableTd>
                    </tr>
                </TableTbody>
            </Table>
        ...
        <!-- Actions -->
        <div v-if="saleReturn.status === 'draft'" class="mt-6 flex justify-end gap-3">
            <Button
                type="link"
                :icon="IconEdit"
...
const handleFinalize = () => {
    if (confirm('Finalize return? Stock will be updated and return will be locked.')) {
        router.post(route('sale-returns.finalize', props.saleReturn.id));
    }
};
</script>
