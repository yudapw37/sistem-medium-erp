<template>
    <DashboardLayout>
        <Head title="Edit Return Penjualan" />

        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Return Penjualan</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Edit transaksi Return Penjualan {{ saleReturn.invoice }}.
                    </p>
                </div>
                <Button
                    type="link"
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div v-if="selectedCustomer" class="p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                            <p class="font-medium text-slate-900 dark:text-white">{{ selectedCustomer.name }}</p>
                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ selectedCustomer.address || '-' }}</p>
                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ selectedCustomer.phone || '-' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 h-full flex flex-col">
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
                        <div class="flex justify-between items-center mb-6">
                            <span class="text-lg font-bold text-slate-700 dark:text-slate-300">Total Return Penjualan</span>
                        ...
                        <div class="flex justify-end gap-3">
                            <Button
                                ...
                                label="Batal"
                                class="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                                @click="router.visit(route('sale-returns.index'))"
                            />
                            <Button
                                type="submit"
...
const form = useForm({
    ...
    items: props.saleReturn?.details?.map(detail => ({
...
const handleSearch = debounce(async () => {
    ...
    try {
        const response = await axios.post(route('sale-returns.searchProduct'), {
            q: searchQuery.value
        });
...
const grandTotal = computed(() => {
    return form.items.reduce((total, item) => total + (item.sell_price * item.qty), 0);
});
...
const submit = () => {
    form.put(route('sale-returns.update', props.saleReturn.id));
};
