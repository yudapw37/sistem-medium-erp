<template>
    <DashboardLayout>
        <Head :title="`Piutang - ${sale.invoice}`" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Detail Piutang</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div class="bg-primary-500 px-6 py-4">
                    <h3 class="text-lg font-bold text-white">Informasi Penjualan</h3>
                </div>
                <div class="p-6 space-y-3">
                    <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                        <span class="text-slate-500">Pelanggan</span>
                        <span class="font-semibold text-slate-900 dark:text-white">{{ sale.customer?.name }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                        <span class="text-slate-500">Tanggal</span>
                        <span class="text-slate-900 dark:text-white">{{ formatDate(sale.created_at) }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                        <span class="text-slate-500">Total</span>
                        <span class="font-mono font-bold text-slate-900 dark:text-white">{{ formatCurrency(sale.grand_total) }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div v-else class="text-center py-8">
                        ...
                        <p class="text-lg font-bold text-green-600">Piutang Lunas</p>
                    </div>
                </div>
            </div>
        </div>
...
const submitPayment = () => {
    form.post(route('receivables.payment', props.sale.id));
};
