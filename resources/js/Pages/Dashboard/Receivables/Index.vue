    ...
    <DashboardLayout>
        ...
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Piutang Usaha</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Kelola piutang dari penjualan tempo
                    </p>
                </div>
                <Button
                    type="link"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
                <p class="text-sm text-slate-500 mb-1">Total Piutang</p>
        ...
        <!-- Receivables Table -->
        <TableCard title="Daftar Piutang">
            <Table>
                <TableThead>
                    <tr>
                ...
                <TableTbody>
                    <template v-if="receivables.data.length > 0">
                        <tr v-for="sale in receivables.data" :key="sale.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                            <TableTd class="font-mono font-bold text-primary-600">{{ sale.invoice }}</TableTd>
                            <TableTd>{{ formatDate(sale.created_at) }}</TableTd>
                            <TableTd>
                                <a :href="route('receivables.customer-card', sale.customer_id)" class="text-primary-600 hover:underline">
                                    {{ sale.customer?.name }}
                                </a>
                            </TableTd>
                            <TableTd class="text-right font-mono">{{ formatCurrency(sale.grand_total) }}</TableTd>
                            <TableTd class="text-right font-mono text-green-600">{{ formatCurrency(sale.paid_amount) }}</TableTd>
                            <TableTd class="text-right font-mono font-bold" :class="sale.remaining > 0 ? 'text-red-600' : 'text-green-600'">
                                {{ formatCurrency(sale.remaining) }}
                            </TableTd>
                            <TableTd>
                                <span :class="[
                                    'px-2 py-1 rounded-full text-xs font-semibold',
                                    sale.is_paid ? 'bg-green-100 text-green-700' :
                                    sale.paid_amount > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                                ]">
                                    {{ sale.is_paid ? 'Lunas' : sale.paid_amount > 0 ? 'Sebagian' : 'Belum Bayar' }}
                                </span>
                            </TableTd>
                            <TableTd class="text-center">
                                <a
                                    :href="route('receivables.show', sale.id)"
                    ...
                    <tr v-else>
                        <TableTd colspan="8" class="text-center py-8">
                            <p class="text-slate-500">Tidak ada piutang</p>
                        </TableTd>
                    </tr>
                </TableTbody>
            </Table>
        ...
        <Pagination v-if="receivables?.links && receivables.links.length > 3" :links="receivables.links" />
    </DashboardLayout>
</template>
