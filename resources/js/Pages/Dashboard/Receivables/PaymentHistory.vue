<template>
    <DashboardLayout>
        <Head title="Riwayat Pembayaran Piutang" />

        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Riwayat Pembayaran Piutang</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Lihat seluruh transaksi pembayaran piutang dari pelanggan.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Pelanggan</label>
                    <select
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <th class="px-6 py-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Invoice Pelanggan</th>
                        ...
                        <tr v-for="payment in payments.data" :key="payment.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex flex-col">
                                    <span class="font-medium text-slate-900 dark:text-white">{{ payment.sale?.invoice }}</span>
                                    <span class="text-xs text-slate-500">{{ payment.customer?.name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm capitalize text-slate-600 dark:text-slate-400">{{ payment.payment_method }}</td>
                            <td class="px-6 py-4 text-sm font-bold text-right text-primary-600 dark:text-primary-400">{{ formatCurrency(payment.amount) }}</td>
...
const handleSearch = debounce(() => {
    router.get(route('receivables.payment-history'), filters, {
        preserveState: true,
        replace: true,
    });
}, 300);
