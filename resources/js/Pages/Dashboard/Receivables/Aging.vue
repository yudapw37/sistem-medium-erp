<template>
    <DashboardLayout>
        <Head title="Aging Piutang" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Aging Report Piutang</h1>
        ...
        <div class="bg-gradient-to-r from-red-500 to-orange-500 rounded-2xl p-6 text-white mb-6">
            <p class="text-red-100 text-sm mb-1">Total Piutang Outstanding</p>
            <p class="text-3xl font-bold font-mono">{{ formatCurrency(totalReceivables) }}</p>
        </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 dark:bg-slate-800">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-semibold text-slate-600">Pelanggan</th>
                            ...
                            <th class="px-4 py-2 text-right text-xs font-semibold text-slate-600">Sisa Piutang</th>
                            <th class="px-4 py-2 text-center text-xs font-semibold text-slate-600">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item in bucket.items" :key="item.sale.id" class="border-b border-slate-100 dark:border-slate-800">
                            <td class="px-4 py-3 font-mono text-primary-600 font-bold">{{ item.sale.invoice }}</td>
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-300">{{ item.sale.customer?.name }}</td>
                            <td class="px-4 py-3 text-slate-500">{{ formatDate(item.sale.created_at) }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <a :href="route('receivables.show', item.sale.id)" class="text-primary-600 hover:underline text-sm">
                                    Bayar
                                </a>
                            </td>
                        </tr>
