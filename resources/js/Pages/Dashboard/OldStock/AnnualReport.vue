<template>
    <DashboardLayout>
        <Head title="Laporan Persediaan Tahunan" />

        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Laporan Persediaan Tahunan</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">
                Pergerakan stok barang berdasarkan data final dari order aktif dan purchase aktif selama satu tahun.
            </p>
        </div>
            ...
            <div class="overflow-x-auto">
                ...
                <div v-else class="flex flex-col items-center justify-center py-16">
                    ...
                    <p class="text-sm text-slate-500">
                        {{ searchQuery ? 'Tidak ditemukan barang dengan pencarian tersebut.' : 'Belum ada data stok untuk tahun ini.' }}
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-start gap-2">
                ...
                <div>
                    ...
                    <ul class="list-disc list-inside space-y-0.5 text-xs text-blue-700 dark:text-blue-400">
                        <li>Stock Awal = stock awal dasar + total masuk sebelum tahun ini − total keluar sebelum tahun ini</li>
                        <li>Stock Masuk = purchase aktif (final) berdasarkan <strong>tanggal faktur</strong> di tahun ini</li>
                        <li>Stock Keluar = order aktif (final) berdasarkan <strong>tanggal order</strong> di tahun ini</li>
                        <li>Stock Akhir tahun ini = Stock Awal tahun berikutnya</li>
                    </ul>
                </div>
            </div>
        </div>
...
const navigate = () => {
    router.get(route('old-stock-report.annual'), {
...
const exportExcel = () => {
    window.location.href = route('old-stock-report.annual.export', {
        year: selectedYear.value,
    });
};
