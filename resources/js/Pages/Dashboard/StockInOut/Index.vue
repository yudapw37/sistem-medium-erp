<template>
    <DashboardLayout>
        <Head title="Stok In/Out" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Stok In/Out</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Kelola penyesuaian stok manual
                    </p>
                </div>
                <Button
                    type="link"
        ...
        <!-- Table -->
        <template v-if="adjustments.data.length > 0">
            <TableCard title="Daftar Penyesuaian Stok">
                <Table>
                    <TableThead>
                        <tr>
                    ...
                    <TableTbody>
                        <tr
                            v-for="(adjustment, i) in adjustments.data"
                            :key="adjustment.id"
                            class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
                        >
                            <TableTd class="text-center">
                                {{ ++i + (adjustments.current_page - 1) * adjustments.per_page }}
                            </TableTd>
                            <TableTd>
                            ...
                            <TableTd>
                                ...
                                >
                                    {{ adjustment.status === 'finalized' ? 'Finalized' : 'Draft' }}
                                </span>
                            </TableTd>
                            <TableTd>
                            ...
                            <TableTd>
                                <div class="flex items-center justify-center gap-2">
                                    <button
                                        @click="router.visit(route('stock-adjustments.show', adjustment.id))"
                                    ...
                                    <template v-if="adjustment.status === 'draft'">
                                        <button
                                            @click="router.visit(route('stock-adjustments.edit', adjustment.id))"
        ...
        <Pagination v-if="adjustments?.links && adjustments.links.length > 3" :links="adjustments.links" />
    </DashboardLayout>
</template>
...
const hasActiveFilters = computed(() => {
    return filterData.value.search || filterData.value.type || filterData.value.warehouse_id;
});

const applyFilters = () => {
    router.get(route('stock-adjustments.index'), filterData.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const handleDelete = (id) => {
    if (confirm('Hapus draf penyesuaian stok ini?')) {
