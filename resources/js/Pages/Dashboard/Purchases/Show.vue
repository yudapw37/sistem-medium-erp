    ...
    <DashboardLayout>
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Detail Pembelian</h1>
        ...
        <TableCard title="Detail Item">
            <Table>
                ...
                <TableTbody>
                    <tr
                        v-for="(item, index) in purchase.details"
                        :key="item.id"
        ...
        <!-- Actions -->
        <div v-if="purchase.status === 'draft'" class="mt-6 flex justify-end gap-3">
            <Button
                type="link"
                :icon="IconEdit"
...
const handleFinalize = () => {
    if (confirm('Finalize purchase? Stock will be updated and purchase will be locked.')) {
        router.post(route('purchases.finalize', props.purchase.id));
    }
};
</script>
