<template>
    <DashboardLayout>
        <Head title="Verifikasi Gudang" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Verifikasi Gudang</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Daftar transaksi menunggu approval dari Gudang
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-4">
            <div class="w-full sm:w-80">
                <Search :url="route('approvals.warehouse.index')" placeholder="Cari No. Invoice..." />
            </div>
        </div>

        <!-- Table -->
        <TableCard title="Pending Warehouse Approval">
            <Table>
                <TableThead>
                    <tr>
                        ...
                        <TableTh>Approved by Finance</TableTh>
                ...
                <TableTbody>
                    <template v-if="sales.data.length > 0">
                        ...
                        >
                            ...
                            <TableTd>
                                <div v-if="sale.finance_approval" class="text-sm">
                                    <span class="text-green-600 font-medium">{{ sale.finance_approval.user?.name }}</span>
                                    <p class="text-xs text-slate-400">{{ formatDate(sale.finance_approval.approved_at) }}</p>
                                </div>
            ...
            <div class="p-6">
                <div class="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl">
                    <p class="text-sm text-green-700 dark:text-green-300">
                        ...
                        Transaksi akan selesai dan berstatus <strong>Completed</strong>.
                    </p>
                </div>
                <div class="mb-4">
...
const submitApprove = () => {
    approvalForm.post(route('approvals.warehouse.approve', selectedSale.value.id), {
        onSuccess: () => {
...
const submitReject = () => {
    rejectForm.post(route('approvals.warehouse.reject', selectedSale.value.id), {
        onSuccess: () => {
