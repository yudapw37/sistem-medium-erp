<template>
    <DashboardLayout>
        <Head title="History Finance Approval" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">History Finance Approval</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Riwayat approval/reject oleh Finance
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-4">
            <div class="flex flex-col sm:flex-row gap-4 items-center flex-wrap">
                <div class="w-full sm:w-64">
                    <Search :url="route('approvals.finance.history')" placeholder="Cari No. Invoice..." />
                </div>
...
const handleFilter = () => {
    router.get(route('approvals.finance.history'), {
        q: props.filters?.q,
        ...filters.value,
    }, { preserveState: true });
};
