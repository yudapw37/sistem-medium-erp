<template>
    <DashboardLayout>
        <Head title="Edit Return Pembelian" />

        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Return Pembelian</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Edit transaksi Return Pembelian {{ purchaseReturn.invoice }}.
                    </p>
                </div>
                <Button
                    type="link"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <!-- Date Selection -->
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
                    <h3 class="font-semibold text-slate-900 dark:text-white mb-4">Tanggal Return</h3>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 h-full flex flex-col">
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
                        <div class="flex justify-between items-center mb-6">
                            <span class="text-lg font-bold text-slate-700 dark:text-slate-300">Total Return Pembelian</span>
                        ...
                        <div class="flex justify-end gap-3">
                            <Button
                                ...
                                label="Batal"
                                class="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                                @click="router.visit(route('purchase-returns.index'))"
                            />
                            <Button
                                type="submit"
...
const form = useForm({
    ...
    date: props.purchaseReturn?.date || new Date().toISOString().split('T')[0],
    grand_total: props.purchaseReturn?.grand_total || 0,
    items: props.purchaseReturn?.details?.map(detail => ({
...
const selectedSupplier  = ref(props.purchaseReturn?.supplier || null);
const selectedPurchase  = ref(props.purchaseReturn?.purchase || null);
const selectedWarehouse = ref(props.purchaseReturn?.warehouse || null);
...
const fetchSupplierPurchases = async (supplierId) => {
    try {
        ...
        // Ensure existing PO appears even if not in list
        if (selectedPurchase.value && !supplierPurchases.value.find(p => p.id === selectedPurchase.value.id)) {
            supplierPurchases.value.push(selectedPurchase.value);
        }
    } catch (error) {
...
onMounted(() => {
    if (selectedSupplier.value) {
        fetchSupplierPurchases(selectedSupplier.value.id);
    }
});
...
const handleSearch = debounce(async () => {
    ...
    try {
        const response = await axios.post(route('purchase-returns.searchProduct'), {
            q: searchQuery.value
        });
...
const submit = () => {
    form.put(route('purchase-returns.update', props.purchaseReturn.id));
};
