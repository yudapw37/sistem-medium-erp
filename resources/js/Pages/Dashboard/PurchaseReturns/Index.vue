<template>
    <DashboardLayout>
        <Head title="Return Pembelian" />

        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Return Pembelian</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400">
                        Riwayat return barang ke supplier
                    </p>
                </div>
                <Button
                    type="link"
        ...
        <div class="mb-4">
            <div class="w-full sm:w-80">
                <Search :url="route('purchase-returns.index')" placeholder="Cari Kode Return..." />
            </div>
        </div>

        <!-- Content -->
        <template v-if="returns.data.length > 0">
            <TableCard title="Riwayat Return Pembelian">
                <Table>
                    <TableThead>
                        <tr>
                        ...
                        >
                            ...
                            <TableTd>
                                <div class="flex items-center justify-center gap-2">
                                    <button
                                        @click="router.visit(route('purchase-returns.show', returnItem.id))"
                                    ...
                                    <button
                                        ...
                                        @click="router.visit(route('purchase-returns.edit', returnItem.id))"
        ...
        >
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">
                Buat return pembelian pertama Anda.
            </p>
            <Button
                type="link"
                :icon="IconCirclePlus"
...
const handleFinalize = (id) => {
    if (confirm('Finalize return? Stock akan dikurangi dan return akan di-lock.')) {
        router.post(route('purchase-returns.finalize', id));
    }
};
