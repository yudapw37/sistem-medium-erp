    ...
    <DashboardLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <IconCoin :size="28" class="text-success-500" />
                        Laporan Keuntungan
                    </h1>
                ...
                <form @submit.prevent="applyFilters">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button
                            ...
                            type="button"
                            @click="resetFilters"
                            class="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 transition-colors"
                        >
                            <IconX :size="18" />
                        </button>
                        <button
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-slate-800">
                                ...
                                <TableTh class="text-right">Penjualan</TableTh>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            ...
                            >
                                <TableTd class="text-sm text-slate-600">
            ...
            >
                ...
                <p class="text-sm text-slate-500">Tidak ada transaksi sesuai filter.</p>
            </div>
...
watch(
    () => props.filters,
    (newFilters) => {
        filterData.value = { ...defaultFilters, ...newFilters };
        selectedCashier.value =
            props.cashiers.find((c) => String(c.id) === newFilters.cashier_id) || null;
        selectedCustomer.value =
            props.customers.find((c) => String(c.id) === newFilters.customer_id) || null;
    },
    { immediate: true }
);
...
const handleSelectCashier = (v) => {
    selectedCashier.value = v;
...
const applyFilters = () => {
    router.get(route('reports.profits.index'), filterData.value, {
        preserveState: true,
        preserveScroll: true,
    });
...
const stats = computed(() => ({
    ...
    average_profit: props.summary?.average_profit ?? 0,
    orders_count: props.summary?.orders_count ?? 0,
    margin: props.summary?.margin ?? 0,
    best_invoice: props.summary?.best_invoice ?? '-',
    best_profit: props.summary?.best_profit ?? 0,
}));
...
const summaryCards = computed(() => [
    {
        title: 'Total Profit',
        value: formatCurrency(stats.value.profit_total),
    ...
    },
    {
        ...
        value: formatCurrency(stats.value.average_profit),
        description: `${stats.value.orders_count} transaksi`,
        icon: IconTrendingUp,
        gradient: 'from-primary-500 to-primary-700',
    },
    {
        title: 'Margin Kotor',
        value: `${stats.value.margin}%`,
    ...
    },
    {
        ...
        value: stats.value.best_invoice,
        description: formatCurrency(stats.value.best_profit),
        icon: IconReceipt,
